...
- **Dependencies to add:** `cpal = "0.17.3"` (only for types in mod.rs re-export — actual stream usage is Plan 02), `rustfft = "6.2"`, `libm = "0.2"`
...
2. Add dependencies to `aucc-rs/Cargo.toml` under `[dependencies]`:
   ...
   libm    = "0.2"
   ```
   Note: cpal 0.17.3 is confirmed available on crates.io. It uses the ALSA host on Linux (compatible with PipeWire via the ALSA layer). APIs used: `default_host()`, `host.devices()`, `device.name()`, `device.default_input_config()`, `device.build_input_stream()`.
