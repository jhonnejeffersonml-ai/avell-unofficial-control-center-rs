...
impl AppState {
    fn new(lb_path: Option<PathBuf>, usb_tx: mpsc::Sender<UsbCmd>, error_rx: mpsc::Receiver<String>) -> Self {
        ...
        match self._mode {
            Some("effect") => {
                let payload = effect_payload(
                    ...
                    self.eff_variant, self.wave_dir, self.reactive, save,
                );
                ...
                match sel {
                    ...
                    "Lightbar — Igual ao teclado" => {
                        if let Some(path) = self.lb_path.clone() {
                            ...
                            let _ = config::save(&LightbarConfig { enabled: true, r, g, b, brightness: self.lb_brightness, save_eeprom: self.save_eeprom });
                    ...
                    s if s.starts_with("💾 Persistir") => {
                        ...
                        if let Ok(cfg) = config::load_file() {
                            let _ = config::save(&config::LightbarConfig { save_eeprom: self.save_eeprom, ..cfg });
                        }
    ...
    fn enable_audio_sync(&mut self) {
        let (r, g, b) = color_at(self.color_a);
        self.audio_snapshot = Some(LedSnapshot { r, g, b, brightness: self.brightness });
...
fn build_list_items(state: &AppState) -> Vec<ListItem<'static>> {
    match state.screen {
        ...
        Screen::AudioSync => {
            ...
            let mut items: Vec<ListItem> = EFFECTS.iter().map(|e| {
...
fn format_uptime(secs: u64) -> String {
    ...
    if days > 0 { format!("{d}d {h}h {m}m", d = days, h = hours, m = mins) }
    else if hours > 0 { format!("{h}h {m}m", h = hours, m = mins) }
    else { format!("{m}m", m = mins) }
}

/// Format KB/s into human-readable throughput.
fn format_throughput(kbs: f32) -> String {
    if kbs >= 1024.0 { format!("{:.1} MB/s", kbs / 1024.0) }
    else if kbs >= 1.0 { format!("{:.0} KB/s", kbs) }
    else { format!("{} B/s", (kbs * 1024.0) as u64) }
}
...
fn render_header_bar(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    let title = Span::styled(" ◈ AUCC DASHBOARD ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD));
    let sys_info = Span::styled(
        format!(" uptime: {} │ load: {:.2} │ perfil: {}", format_uptime(t.system.uptime_secs), t.system.load_avg_1, prof),
        Style::default().fg(Color::DarkGray),
    );
    let refresh_info = Span::styled(
        "auto-refresh: 1s",
        Style::default().fg(Color::DarkGray),
    );
...
fn render_footer_bar(f: &mut Frame, state: &AppState, area: Rect) {
    let Some(ref t) = state.telemetry else {
        ...
        f.render_widget(Paragraph::new(help)
            .style(Style::default().fg(Color::DarkGray))
    ...
    // CPU
    let cpu_util = if t.cpu.temp_avg > 40.0 { (t.cpu.temp_avg - 40.0) / 60.0 * 100.0 } else { 0.0 };
    parts.push(Span::styled(format!("CPU {:.0}%", cpu_util), Style::default().fg(Color::Cyan)));
    parts.push(Span::styled(format!("│{:.0}°C", t.cpu.temp_max), Style::default().fg(Color::Red)));

    // GPU
    if let Some(ref g) = t.gpu {
        parts.push(Span::raw("  "));
        parts.push(Span::styled(format!("GPU {}%", g.utilization_pct), Style::default().fg(Color::Magenta)));
        parts.push(Span::styled(format!("{:.0}°C", g.temp_c), Style::default().fg(Color::Yellow)));
    }

    // RAM
    parts.push(Span::raw("  "));
    parts.push(Span::styled(format!("RAM {:.0}%", t.ram.used_pct), Style::default().fg(Color::Green)));

    // Battery
    if let Some(ref b) = t.battery {
        parts.push(Span::raw("  "));
        let bat_icon = if b.current_now_ma > 0 { "⚡" } else if b.current_now_ma < 0 { "↓" } else { "=" };
        parts.push(Span::styled(format!("BAT {}%{}", b.capacity_pct, bat_icon), Style::default().fg(Color::Yellow)));
    }

    // Uptime
    parts.push(Span::raw("  "));
    parts.push(Span::styled(format!("UP {}", format_uptime(t.system.uptime_secs)), Style::default().fg(Color::DarkGray)));

    // Help text
    parts.push(Span::raw("  │  "));
    parts.push(Span::styled("Enter=atualizar  ESC=voltar  q=sair", Style::default().fg(Color::DarkGray)));
...
fn render_cpu_card(f: &mut Frame, state: &AppState, area: Rect) {
    let Some(ref t) = state.telemetry else { return; };
    ...
    let sparkline_data: Vec<u64> = if !state.telemetry_history.cpu_temp.is_empty() {
        let max_temp = state.telemetry_history.cpu_temp.iter().cloned().fold(0f32, f32::max).ceil().max(1.0);
        state.telemetry_history.cpu_temp.iter()
            .map(|v| (*v / max_temp * 10.0) as u64).collect()
    } else {
        Vec::new()
    };
    ...
    let constraints = if has_spark {
        vec![Constraint::Length(1), Constraint::Length(1), Constraint::Length(1), Constraint::Length(1), Constraint::Length(1)]
    } else {
    ...
    f.render_widget(Paragraph::new(Line::from(vec![
        ...
        Span::raw("    "),
        Span::styled(format!("{} cores", t.cpu.core_count), Style::default().fg(Color::DarkGray)),
    ])), chunks[0]);

    f.render_widget(Paragraph::new(Line::from(vec![
        Span::styled(" TEMP ", Style::default().fg(Color::DarkGray)),
        Span::styled(format!("{:.0}°/{:.0}°", t.cpu.temp_avg, t.cpu.temp_max), Style::default().fg(color).add_modifier(Modifier::BOLD)),
    ])), chunks[1]);

    f.render_widget(Gauge::default()
        .gauge_style(Style::default().fg(color))
        .percent(temp_pct.min(100.0) as u16)
        .label(format!("{:.0}°C", t.cpu.temp_avg)), chunks[1]);

    f.render_widget(Gauge::default()
        .gauge_style(Style::default().fg(color))
        .percent(cpu_util.min(100.0) as u16)
        .label(format!("{:.0}% util", cpu_util)), chunks[2]);
...
fn render_gpu_card(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    if let Some(ref g) = t.gpu {
        ...
        f.render_widget(Paragraph::new(Line::from(vec![
            ...
            Span::styled(short_name, Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)),
        ])), chunks[0]);

        f.render_widget(Paragraph::new(Line::from(vec![
            Span::styled(" TEMP ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{}°C", g.temp_c), Style::default().fg(temp_color).add_modifier(Modifier::BOLD)),
        ])), chunks[1]);
        f.render_widget(Gauge::default()
            .gauge_style(Style::default().fg(temp_color)).percent(temp_pct as u16), chunks[1]);

        f.render_widget(Gauge::default()
            .gauge_style(Style::default().fg(util_color))
            .percent(g.utilization_pct as u16)
            .label(format!("{}% util", g.utilization_pct)), chunks[2]);

        f.render_widget(Gauge::default()
            .gauge_style(Style::default().fg(vram_color))
            .percent(vram_pct as u16)
            .label(format!("{}/{} MB", g.vram_used_mb, g.vram_total_mb)), chunks[3]);
    } else {
        f.render_widget(Paragraph::new(Line::from(Span::styled("  GPU indisponível", Style::default().fg(Color::DarkGray)))).alignment(ratatui::layout::Alignment::Center), inner);
    }
}
...
fn render_ram_card(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    let spark_data: Vec<u64> = if !state.telemetry_history.ram_usage_pct.is_empty() {
        state.telemetry_history.ram_usage_pct.iter().map(|v| (*v / 10.0) as u64).collect()
    } else { Vec::new() };
    ...
    let n_constraints = if has_swap {
        if has_spark { 5 } else { 4 }
    } else {
        if has_spark { 4 } else { 3 }
    };
    let constraints = vec![Constraint::Length(1); n_constraints];
    let chunks = Layout::default().direction(LayoutDir::Vertical).constraints(constraints).split(inner);

    let mut idx = 0;
    f.render_widget(Paragraph::new(Line::from(vec![
        Span::styled(" MEM  ", Style::default().fg(Color::DarkGray)),
        Span::styled(format!("{}/{} ({:.0}%)", t.ram.used_mb, t.ram.total_mb, ram_pct), Style::default().fg(ram_color).add_modifier(Modifier::BOLD)),
        Span::raw("  "),
        Span::styled(format!("{} MB disp.", t.ram.available_mb), Style::default().fg(Color::DarkGray)),
    ])), chunks[idx]); idx += 1;

    f.render_widget(Gauge::default()
        .gauge_style(Style::default().fg(ram_color))
        .percent(ram_pct.min(100.0) as u16)
        .label(format!("{:.0}% usada", ram_pct)), chunks[idx]); idx += 1;
    ...
    if has_swap {
        ...
        f.render_widget(Paragraph::new(Line::from(vec![
            ...
            Span::styled(format!("{}/{} MB ({:.0}%)", t.ram.swap_used_mb, t.ram.swap_total_mb, swap_pct), Style::default().fg(swap_color)),
        ])), chunks[idx]); idx += 1;
        f.render_widget(Gauge::default()
            .gauge_style(Style::default().fg(swap_color))
            .percent(swap_pct.min(100.0) as u16), chunks[idx]); idx += 1;
    }
...
fn render_network_card(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    if let Some(ref n) = t.network {
        ...
        f.render_widget(Paragraph::new(Line::from(vec![
            ...
            Span::styled(&n.interface, Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD)),
        ])), chunks[0]);

        f.render_widget(Paragraph::new(Line::from(vec![
            Span::styled(" ↓ RX ", Style::default().fg(Color::Green)),
            Span::styled(format!("{:>10}", rx_str), Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            Span::raw("   "),
            Span::styled("↑ TX ", Style::default().fg(Color::Cyan)),
            Span::styled(format!("{:>10}", tx_str), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ])), chunks[1]);

        f.render_widget(Paragraph::new(Line::from(vec![
            Span::styled(" TOT  ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("↓{:.0}MB", total_rx_mb), Style::default().fg(Color::Green)),
            Span::raw("    "),
            Span::styled(format!("↑{:.0}MB", total_tx_mb), Style::default().fg(Color::Cyan)),
        ])), chunks[2]);
    } else {
        f.render_widget(Paragraph::new(Line::from(Span::styled("  Sem dados de rede", Style::default().fg(Color::DarkGray)))).alignment(ratatui::layout::Alignment::Center), inner);
    }
}
...
fn render_nvme_battery_card(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    if has_nvme {
        for nvme in &t.nvme {
            ...
            f.render_widget(Paragraph::new(Line::from(vec![
                Span::styled(format!(" {}  ", nvme.hwmon), Style::default().fg(Color::DarkGray)),
                Span::styled(label, Style::default().fg(color).add_modifier(Modifier::BOLD)),
            ])), chunks[idx]); idx += 1;
            f.render_widget(Gauge::default()
                .gauge_style(Style::default().fg(color))
                .percent(temp_pct as u16), chunks[idx]); idx += 1;
        }
    } else {
        f.render_widget(Paragraph::new(Line::from(Span::styled("  NVMe não detectado", Style::default().fg(Color::DarkGray)))), chunks[idx]); idx += 1;
    }
    ...
    if let Some(ref b) = t.battery {
        ...
        let time_str = b.time_remaining_min.map(|m| {
            if m >= 60 { format!("{h}h {m}min", h = m / 60, m = m % 60) }
            else { format!("{m} min", m = m) }
        }).unwrap_or_else(|| "—".into());
        let power_str = b.power_w.map(|w| format!("{w:.1}W")).unwrap_or_else(|| "—".into());

        // Separator line
        f.render_widget(Paragraph::new(Span::styled(" ─────────────────────────", Style::default().fg(Color::DarkGray))), chunks[idx]); idx += 1;

        f.render_widget(Paragraph::new(Line::from(vec![
            Span::styled(" BAT  ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{}%  {}", b.capacity_pct, direction), Style::default().fg(color).add_modifier(Modifier::BOLD)),
        ])), chunks[idx]); idx += 1;

        f.render_widget(Gauge::default()
            .gauge_style(Style::default().fg(color))
            .percent(cap_pct as u16)
            .label(format!("{}%  {}  {}", b.capacity_pct, time_str, power_str)), chunks[idx]); idx += 1;

        f.render_widget(Paragraph::new(Line::from(vec![
            Span::styled(" CARGA", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{}/{} mAh", b.charge_now_mah, b.charge_full_mah), Style::default().fg(Color::DarkGray)),
            Span::raw("  "),
            Span::styled(format!("ciclos: {}", b.cycle_count), Style::default().fg(Color::DarkGray)),
        ])), chunks[idx]); idx += 1;
    } else {
        if has_nvme {
            f.render_widget(Paragraph::new(Span::styled(" ─────────────────────────", Style::default().fg(Color::DarkGray))), chunks[idx]); idx += 1;
        }
        f.render_widget(Paragraph::new(Line::from(Span::styled("  Bateria não detectada", Style::default().fg(Color::DarkGray)))), chunks[idx]);
    }
}
...
fn render_processes_card(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    if procs.is_empty() {
        f.render_widget(Paragraph::new(Line::from(Span::styled("  Nenhum processo", Style::default().fg(Color::DarkGray)))).alignment(ratatui::layout::Alignment::Center), inner);
        return;
    }

    // Header
    let header = Line::from(vec![
        Span::styled(" PID    ", Style::default().fg(Color::DarkGray).add_modifier(Modifier::BOLD)),
        Span::styled(" NOME           ", Style::default().fg(Color::DarkGray).add_modifier(Modifier::BOLD)),
        Span::styled(" CPU%  ", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
        Span::styled(" MEM%", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
    ]);

    let mut items = vec![header];
    for p in procs {
        let cpu_color = if p.cpu_pct >= 80.0 { Color::Red } else if p.cpu_pct >= 40.0 { Color::Yellow } else { Color::White };
        let name = if p.name.len() > 14 { format!("{}...", &p.name[..11]) } else { p.name.clone() };
        items.push(Line::from(vec![
            ...
            Span::styled(format!(" {:>4.1}", p.mem_pct), Style::default().fg(Color::Green)),
        ]));
    }
...
fn render_network_chart(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    if h.net_rx_kbs.is_empty() || h.net_rx_kbs.len() < 2 {
        f.render_widget(Paragraph::new(Line::from(Span::styled("  Aguardando dados...", Style::default().fg(Color::DarkGray)))).alignment(ratatui::layout::Alignment::Center), inner);
        return;
    }

    let rx_data: Vec<(f64, f64)> = h.net_rx_kbs.iter()
        .enumerate().map(|(i, v)| (i as f64, *v as f64)).collect();
    let tx_data: Vec<(f64, f64)> = h.net_tx_kbs.iter()
        .enumerate().map(|(i, v)| (i as f64, *v as f64)).collect();

    let max_rx = h.net_rx_kbs.iter().cloned().fold(0f32, f32::max).ceil().max(1.0);
    let max_tx = h.net_tx_kbs.iter().cloned().fold(0f32, f32::max).ceil().max(1.0);
...
fn render_cpu_chart(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    if h.cpu_temp.is_empty() || h.cpu_temp.len() < 2 {
        f.render_widget(Paragraph::new(Line::from(Span::styled("  Aguardando dados...", Style::default().fg(Color::DarkGray)))).alignment(ratatui::layout::Alignment::Center), area);
        return;
    }

    let temp_data: Vec<(f64, f64)> = h.cpu_temp.iter()
        .enumerate().map(|(i, v)| (i as f64, *v as f64)).collect();
    let util_data: Vec<(f64, f64)> = h.cpu_utilization.iter()
        .enumerate().map(|(i, v)| (i as f64, *v as f64)).collect();

    let max_temp = h.cpu_temp.iter().cloned().fold(0f32, f32::max).ceil().max(1.0);
...
fn render_ram_chart(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    if h.ram_usage_pct.is_empty() || h.ram_usage_pct.len() < 2 {
        let text = vec![Line::from(vec![
            Span::styled("  Aguardando dados...", Style::default().fg(Color::DarkGray)),
        ])];
    ...
    let ram_data: Vec<(f64, f64)> = h.ram_usage_pct.iter()
        .enumerate()
        .map(|(i, v)| (i as f64, *v as f64))
        .collect();

    let max_ram = h.ram_usage_pct.iter().cloned().fold(0f32, f32::max).ceil().max(1.0);
            ...
            if let Some(ref d) = dev {
                let result = match cmd {
                    ...
                    UsbCmd::Effect(payload) => {
                        d.apply_effect(&payload)
                    }
                    UsbCmd::Disable => {
                        d.disable()
                    }
                    UsbCmd::LbColor { path, r, g, b, brightness } => {
                        lightbar::apply_color(&path, r, g, b, brightness).map_err(|_| rusb::Error::Io)
                    }
