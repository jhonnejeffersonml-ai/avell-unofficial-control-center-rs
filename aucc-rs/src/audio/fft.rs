...
fn get_fft() -> Arc<dyn rustfft::Fft<f32>> {
    Arc::clone(FFT.get_or_init(|| FftPlanner::<f32>::new().plan_fft_forward(FFT_SIZE)))
}
...
pub fn process_fft(mono_samples: &[f32], sample_rate: u32) -> Option<BandAmplitudes> {
    ...
    let mut buffer: Vec<Complex<f32>> = slice
        ...
        .map(|(i, &s)| {
            let window = 0.5
                * (1.0 - libm::cosf(2.0 * std::f32::consts::PI * i as f32 / (FFT_SIZE - 1) as f32));
            Complex {
                re: s * window,
                im: 0.0,
            }
    ...
    let rms = |start: usize, end: usize| -> f32 {
        ...
        let sum_sq: f32 = buffer[start..end].iter().map(|c| c.norm_sqr()).sum();
...
mod tests {
    ...
    fn bass_dominates_for_low_freq_sine() {
        ...
        let bands = process_fft(&samples, 44100).unwrap();
        assert!(
            bands.bass > bands.mid,
            "bass={} should > mid={}",
            bands.bass,
            bands.mid
        );
        assert!(
            bands.bass > bands.treble,
            "bass={} should > treble={}",
            bands.bass,
            bands.treble
        );
    ...
    fn treble_dominates_for_high_freq_sine() {
        ...
        let bands = process_fft(&samples, 44100).unwrap();
        assert!(
            bands.treble > bands.bass,
            "treble={} should > bass={}",
            bands.treble,
            bands.bass
        );
        assert!(
            bands.treble > bands.mid,
            "treble={} should > mid={}",
            bands.treble,
            bands.mid
        );
    ...
    fn smoothing_converges_release() {
        let mut current = BandAmplitudes {
            bass: 1.0,
            mid: 1.0,
            treble: 1.0,
        };
    ...
    fn smoothing_attack_faster_than_release() {
        ...
        let mut falling = BandAmplitudes {
            bass: 1.0,
            mid: 1.0,
            treble: 1.0,
        };
        let up_target = BandAmplitudes {
            bass: 1.0,
            mid: 1.0,
            treble: 1.0,
        };
        ...
        assert!(
            rising.bass > (1.0 - falling.bass),
            "attack movement {} should > release movement {}",
            rising.bass,
            1.0 - falling.bass
        );
    }
    ...
    fn uses_last_fft_size_samples() {
        ...
        assert!(
            bands.bass > 0.01,
            "should detect the sine in the last window"
        );
    }
}
